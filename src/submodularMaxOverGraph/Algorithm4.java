...
public class Algorithm4<V> {
	...
	public static <V> ArrayList <V> greedyWalk(int mode, Graph<V> road_network, InfluenceModel<V> social_network, 
		...
		while (!unvisited_set.isEmpty()){
			...
			if(mode==1) cost_cur+=Driver_dtd.visit_cost*exclude(visited_set, iniActiveNodes).size();//cost before choosing a new vertex, i.e., approximate shortest walk covering all nodes
	...
	public static <V> ArrayList<V> greedyWalk_sp (Graph<V> sensorNet,
		...
		//ADD all nodes to unvisited set if not initially visited
		...
		while (!unvisited_set.isEmpty()){
			...
			double fc_max_iter=Double.NEGATIVE_INFINITY; //MAX influence for this iteration
			double delta_f_max=0; //MAX influence change for this iteration
			double cost_best_iter=0; //BEST cost by adding BEST node
			...
			double cost_cur;
			if(USE_CHRISTOFIDES)
				cost_cur=Graph.shortestCoverCostChristofides(start,sensorNet,exclude(visited_set,s)).length; //[aij]	
			else
				cost_cur=Graph.shortestCoverCostGreedy(start,sensorNet,exclude(visited_set, s)).length;//cost before choosing a new vertex, i.e., approximate shortest walk covering all nodes
			
			
			cost_cur+=Driver_sp.visit_cost*exclude(visited_set, s).size();
			...
			for(V v:unvisited_set){	
				...
				//System.out.println(exclude(u,s));
				//Walk and cost
				WrapWalkWithLength walk_new;
				if(USE_CHRISTOFIDES)
			...
			if(node_best_iter!=null&&cost_best_iter<=budget){ //method 0
				//if (node_best_iter!=null){  //method 1
				//System.out.println("node_best_iter="+node_best_iter);
				walk_best=walk_best_iter;
				visited_set.add(node_best_iter); //ADD best node to visited set
				unvisited_set.remove(node_best_iter); //REMOVE best node from unvisited set
				//System.out.println("visited set:"+visited_set+";cost:"+cost_new_max+";walk:"+walk_new_max);
				Driver_sp.max_entropy+=delta_f_max; //Track influence changes
				...
				//System.out.println("delta entropy="+delta_f_max);
			}else break;			
		}
